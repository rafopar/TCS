...
\end{figure}
 ...
 It is important to mention that in the above described method, integration limits over $\theta$ should not be changed for different $\phi$ bins. Varying integration limits, the $\phi$ dependence of the weighted cross section will not have a cosine shape anymore.
...
Then using equations (\ref{eq:Int_eq_Tot_minus_BH}) and (\ref{eq:extrapolation}), the interference term WCRS will be calculated for any $\theta\in(a-b)$ range for any given $\phi$ bin.
 ...
 In order to demonstrate steps towards the extraction of the $ReM^{--}$, let's assume assume that we have divided the $(0, 2\pi)$ region into 10 bins in $\phi$, and for each of $\phi$ bin we have measured the WCRS
 \begin{equation}
  \frac{dS}{dQ^{\prime 2} dt d\phi} = \frac{\dstl \sum_{i = 1}^{N_{bin}} \frac{L_{i}}{L_{0\;i}}}{{\cal{L}}\cdot Acc \cdot \Delta Q^{\prime 2} \Delta t \Delta \phi}
...
In the 2nd step the we should subtract BH WCRS from the measured WCRS. The subtracted WCRS is shown in Fig.\ref{fig:crs_sections_plot2}. This distribution is fitted with a $P\cdot cos(\phi)$ function (the red curve). As $P$ will be obtained from the fit, the $ReM^{--}$ can be calculated directly using the eq. (\ref{eq:Re_M--}).
