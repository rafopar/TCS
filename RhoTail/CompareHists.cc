...
namespace {
    ...
    const unordered_map<string, string> HistTitles = {
        ...
        {"h_MC_Minv_pippim_All", "MC truth M(#pi^{+}#pi^{-}), all events;M(#pi^{+}#pi^{-}) (GeV);Counts"},
        {"h_MC_Minv_pippim_RecSel", "MC truth M(#pi^{+}#pi^{-}), rec. selected;M(#pi^{+}#pi^{-}) (GeV);Counts"},
        {"h_MC_Minv_pippim_Mx2Cut", "MC truth M(#pi^{+}#pi^{-}), M_{x}^{2} cut;M(#pi^{+}#pi^{-}) (GeV);Counts"},
